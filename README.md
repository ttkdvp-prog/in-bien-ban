...
3. Điền các thuộc tính như sau:
   ...
   - **Target:** Sử dụng bộ Formula gộp chuỗi chuẩn như sau:
     ```text
     CONCATENATE("https://[thay_bang_domain_vercel_cua_ban]/form_in.html?id=", [Id_BienBan])
     ```
     *(Lưu ý: Domain thực tế có dạng `suco-vnpt.vercel.app`, nhớ giữ nguyên đuôi `/form_in.html?id=`. Link Vercel này sẽ được lấy khi bạn triển khai xong Bước 3).*
